...
const TYPE_COLORS = {
  Crisis: "bg-[#ffdad6]/60 text-[#ba1a1a]",
  Counselling: "bg-[#e4dfff]/60 text-[#5742d3]",
  Emergency: "bg-[#ffdad6]/80 text-[#93000a]",
  "Suicide Prevention": "bg-[#ffdcc2]/60 text-[#8a4c05]",
  "Mental Health": "bg-[#75f9d3]/30 text-[#006b56]",
};

export default function Helplines() {
  return (
    <AppLayout title="Crisis Helplines" subtitle="You're not alone — help is available right now">
      <div className="space-y-5">
        ...
        <GlassCard className="p-5 bg-[#ffdad6]/40 border border-[#ffdad6]">
          <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center">
            ...
            <div className="flex-1">
              <h3 className="font-display text-lg font-bold text-[#93000a]">In immediate danger?</h3>
              <p className="text-sm text-[#93000a]/80 mt-0.5">Call 112 for emergency services or reach out to any helpline below.</p>
            </div>
            <a
              href="tel:112"
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
          {CRISIS_LINES.map((line) => (
            <GlassCard key={line.name} className="p-5 flex flex-col gap-3 hover:scale-[1.01] transition-transform duration-200">
              <div className="flex items-start justify-between">
                <div className={`p-2 rounded-xl ${line.bg}`}>
                  <span className={`material-symbols-outlined ${line.color}`}>call</span>
                </div>
                <span className={`text-[10px] font-mono px-2 py-1 rounded-full ${TYPE_COLORS[line.type] || "bg-[#e7eeff] text-[#787586]"}`}>
                  {line.type}
                </span>
              </div>
              <div>
                <h4 className="font-semibold text-[#111c2d] text-sm">{line.name}</h4>
                <p className="text-xs text-[#787586] mt-0.5 leading-relaxed">{line.desc}</p>
              </div>
              <div className="mt-auto space-y-2">
                ...
                <p className="text-center text-[10px] text-[#787586] font-mono">{line.available}</p>
              </div>
            </GlassCard>
          ))}
        </div>
        ...
        {/* Reminder */}
        <GlassCard className="p-5 flex gap-3 items-start">
          <span className="material-symbols-outlined text-[#5742d3] shrink-0">info</span>
          <p className="text-sm text-[#474554] leading-relaxed">
            These helplines are operated by trained professionals. Calls are confidential.
            If you're supporting someone in crisis, you can also reach out on their behalf.
